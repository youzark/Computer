...
if __name__ == '__main__':
    ...
    print("clients and server initialization...",flush=True)
    ...
    for iter in range(args.epochs):
        ...
        print('Round {:3d}, Training average loss {:.3f}'.format(iter, loss_glob),flush=True)
        print("Round {:3d}, Testing accuracy: {:.2f}".format(iter, acc_test),flush=True)
        
    # testing

    acc_train, loss_train = server.test(dataset_train)
    acc_test, loss_test = server.test(dataset_test)
    print("Training accuracy: {:.2f}".format(acc_train),flush=True)
    print("Testing accuracy: {:.2f}".format(acc_test),flush=True)
